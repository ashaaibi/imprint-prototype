...
## Page map
- `index.html` — **marketplace landing** (three-sided platform: artists · brands · makers; featured
  products/artists/collections/makers, how-it-works). Data-driven from `catalog.js`.
- `products.html` — **category-organised browse** (sticky category chips, `#category` deep-links).
- `<slug>/index.html` — **per-product clean-URL PDPs** (e.g. `/honeyloom-gift-bag/`, `/coffee-cup/`).
  Stubs generated by `tools/build_catalog.py`; non-flagship ones render via `product.js` (lean PDP).
  Honeyloom is a hand-built **rich** PDP (ported from the old `product.html`). `product.html` now
  **redirects** to `/honeyloom-gift-bag/`.
- `artists.html` — artist directory + per-artist profile (`?a=<id>`); `manufacturers.html` — maker
  directory + profile (`?m=<id>`); `collections.html` — all collections.
...
```bash
python3 tools/build_catalog.py    # re-run after editing the data or assets
```
- **Clean URLs**: product pages live at `/<slug>/` (one level deep). Each stub injects a runtime
  `<base href>` (computed from `location.pathname`) so `shared.css`/`catalog.js`/relative nav all resolve
  to the site root — `shared.js` works unmodified. **Keep detail pages exactly one level deep** or the
  base shim breaks. Artists/makers use query-param pages (`artists.html?a=`, `manufacturers.html?m=`).
- **`app.js`** renders the secondary nav (`#site-nav` via `IMP.initAppHeader`) + footer (`#site-footer`),
  holds favourites (`imprint_favs`) / follows (`imprint_follows`) in localStorage, and exposes card render
  helpers (`IMP.productCard/artistCard/makerCard/collectionCard/tagRow`). Hearts/follow/share use document
  delegation (`[data-fav]`, `[data-follow]`, `[data-share]`). Marketplace pages call `IMP.initAppHeader`;
  the configurator/checkout keep plain `initSiteHeader` (untouched).
- **Honeyloom** is the flagship: hand-built rich PDP at `/honeyloom-gift-bag/`, real configurator template.
  Its PDP reads the template's `locks.sizes` and **disables locked-out sizes on the page** (not just in the
  studio). Other products are **locked placeholder templates** rendered by `product.js`.
